...
int createSocket(USocket* mySock, Address* myAddr, int broadcast){
	if((*mySock = socket(AF_INET, SOCK_DGRAM, 0))== -1) 
		return(-1);
	if(broadcast == 1){
		int enableBroadcast = 1;
		setsockopt(*mySock, SOL_SOCKET, SO_BROADCAST, &enableBroadcast, sizeof(enableBroadcast));
	}
	if(bind(*mySock, (struct sockaddr*)myAddr, (socklen_t)sizeof(Address))) 
		return(-2);
...
int receiveMessage(USocket mySock, char** msg, Address* mittente){
	...
	ret = recvfrom(mySock, buffer, MAX_LENGTH, 0, (struct sockaddr*)mittente, (socklen_t*)&len_addr);
	buffer[ret] = '\0';
	*msg = strdup(buffer);
